...
#---------------------------------------------
def calcLimits(CHANNEL_NAME, COMB_DATACARD_NAME, CAT_DIR, MASS):
    ...
    execme('combine --rAbsAcc 0.000001 '+t2wDataCardName+' -M AsymptoticLimits --mass '+str(MASS)+' --name ChargedHiggs_'+CHANNEL_NAME+'_'+CAT_DIR)
...
def getCardsToBeCombined(CHANNEL_ARRAY, IN_FILE_DIR_ARRAY, HIST_ARRAY, MASS):
    ...
    for CH in range(len(CHANNEL_ARRAY)):
        ...
        for HIST in range(len(HIST_ARRAY)):
            COMB_CARD_MASS = []
            COMB_CARD_MASS.append('datacard_csbar_'+CHANNEL_ARRAY[CH]+'_'+HIST_ARRAY[HIST][0]+'_'+HIST_ARRAY[HIST][1]+'_13TeV_WH'+str(MASS)+'.txt')
...
#---------------------------------------------------
def calcCombinedLimit(CHANNEL_ARRAY, IN_FILE_DIR_ARRAY, HIST_ARRAY, CAT_DIR, MASS_ARRAY, isCondSub):
    ...
    for MASS in range(len(MASS_ARRAY)):
        ...
        COMB_DATACARD_NAME = 'combine_datacard_csbar_'+COMB_CHANNEL_NAME+"_"+CAT_DIR+"_13TeV_WH"+str(MASS_ARRAY[MASS])+".txt"
        ...
        calcLimits(COMB_CHANNEL_NAME, COMB_DATACARD_NAME, CAT_DIR, MASS_ARRAY[MASS])
...
if __name__=="__main__":
    ...
    hist_array_CTagCat_bJetPtCat.append(["nonKinFit", "mjj_non_kfit"])
    ...
    if(int(args.cat)==1):
        ...
        cat_dir = "Cat1_Inc"
    if(int(args.cat)==2):
        in_hist = hist_array_bJetPtCat
        cat_dir = "Cat2_bJetPtBin"
    if(int(args.cat)==3):
        in_hist = hist_array_CTagL
        cat_dir = "Cat3_cTagInc"
    if(int(args.cat)==4):
        in_hist = hist_array_CTagCat
        cat_dir = "Cat4_cTagEx"
    if(int(args.cat)==5):
        in_hist = hist_array_CTagCat_bJetPtCat
        cat_dir = "Cat5_cTagEx_bJetPtBin"
