...
baseDir="/home/rverma/t3store2/condor_out/limit_condor/limit_log/"
...
do
  ...
  IFS='_' read -r -a array <<< "$ntupleT2Path"
  len=${#array[@]}
  mass="${array[0]}"
  channel="${array[1]}"
  cat="${array[2]}"

  mkdir -p /home/rverma/t3store2/condor_out/limit_condor/limit_out/$channel/$cat
  #echo $ntuple
  iFile=$channel$cat$mass
