...
def execme(command):
    ...
    os.system(command)
execme("mkdir -p ~/ARCHIVE/limit2D")
execme("cp *.C ~/ARCHIVE/limit2D")
...
def makeDataCards(CHANNEL_NAME, IN_FILE_DIR, HIST_DIR, HIST_NAME):
    print HIST_NAME
    print HIST_DIR
    #execme("cp ~/ARCHIVE/limit2D/*.C .")
    execme("cp makeBinnedHPlusDataCard_13TeV.C ../")
    execme("cp template_datacard_csbar.log ../")
    execme('sed -i '+'s#inFileDir_#'+IN_FILE_DIR+'# makeBinnedHPlusDataCard_13TeV.C')
    execme('sed -i '+'s#inShapeHisto_#'+HIST_NAME+'# makeBinnedHPlusDataCard_13TeV.C')
    execme('sed -i '+'s#inShapeHistoDir_#'+HIST_DIR+'# makeBinnedHPlusDataCard_13TeV.C')
    if(CHANNEL_NAME=="mu"):
        execme('sed -i '+'s#isMuonChannel_#true# makeBinnedHPlusDataCard_13TeV.C')
        execme('sed -i '+'s#isEleChannel_#false# makeBinnedHPlusDataCard_13TeV.C')
        execme('cp '+muon_file_dir+'/all_muData.root '+muon_file_dir+'/all_Data.root')
        execme('sed -i '+'s#LUMI#'+str(muon_lumi)+'# makeBinnedHPlusDataCard_13TeV.C')
    if(CHANNEL_NAME=="ele"):
        execme('sed -i '+'s#isMuonChannel_#false# makeBinnedHPlusDataCard_13TeV.C')
        execme('sed -i '+'s#isEleChannel_#true# makeBinnedHPlusDataCard_13TeV.C')
        execme('cp '+ele_file_dir+'/all_EleData.root '+ele_file_dir+'/all_Data.root')
        execme('sed -i '+'s#LUMI#'+str(ele_lumi)+'# makeBinnedHPlusDataCard_13TeV.C')
    execme('sed -i '+'s#CHANNEL#'+CHANNEL_NAME+'# makeBinnedHPlusDataCard_13TeV.C')
    execme('sed -i '+'s#CHANNEL#'+CHANNEL_NAME+'# template_datacard_csbar.log')
    execme('root -b -q makeBinnedHPlusDataCard_13TeV.C')
    execme("mv ../makeBinnedHPlusDataCard_13TeV.C . ")
...
def calcLimits(CHANNEL_NAME, HIST_NAME, MASS_ARRAY, MY_LIMIT_DIR):
    for MASS_POINT in MASS_ARRAY:
        ...
        execme('combine --rAbsAcc 0.000001 t2w_'+DATACARD+'.root -M Asymptotic --mass '+str(MASS_POINT)+' --name ChargedHiggs_'+CHANNEL_NAME+'_'+MY_LIMIT_DIR)
...
muon_lumi = 35.50
ele_lumi = 35.50
muon_file_dir="stack_20180116_Mu_sys_bin10"
ele_file_dir="stack_20180116_Ele_sys_bin10"
mu_channel_array = ["mu"]
...
hist_array_bJetPtCat.append(["PtbJetInc", "mjj_kfit_00To35"])
...
hist_array_CTagCat_bJetPtCat.append(["PtbJetCatL", "mjj_kfit_00To35"])
...
hist_array_CTagCat_bJetPtCat.append(["PtbJetCatM", "mjj_kfit_00To35"])
...
hist_array_CTagCat_bJetPtCat.append(["PtbJetCatT", "mjj_kfit_00To35"])
