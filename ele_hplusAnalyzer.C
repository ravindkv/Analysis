...
void hplusAnalyzer::CutFlowProcessor(TString url,  string myKey, TString cutflowType, TFile *outFile_){
  ...
  for(int i=0; i<nEntries; ++i){
    ...
    if(isIso20){
      if(tmp_iso <= 0.13) cutflowType_ = cutflowType+"/Iso";
      if(tmp_iso > 0.13 && tmp_iso <= 0.35) cutflowType_ = cutflowType+"/NonIso";
    }
    else{
    ...
    bool maxIso = false;
    if(isIso20){ 
      if(tmp_iso > 0.35) maxIso = true;
    }
    else if(tmp_iso > 0.30) maxIso = true;
    if(maxIso) continue;
    nCutPass = 5;
