...
def doMaxLikeFit(limit_dir, channel, hist, mass):
    ...
    execme('combine '+t2w_file+' -M FitDiagnostics -t -1 --expectSignal 1 --robustFit 1 | tee '+combineLogFile, True)
    #execme('combine '+t2w_file+' -M FitDiagnostics -t -1 --expectSignal 1 | tee '+combineLogFile, True)
